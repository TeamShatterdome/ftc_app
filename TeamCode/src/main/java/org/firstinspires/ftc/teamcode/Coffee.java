...
public class Coffee extends OpMode {
    ...
    public void mechanum(){
        if (gamepad1.right_stick_button) {
            ...
            robot.frontLeft.setPower((0.5) * gamepad1.left_stick_y);
            robot.frontRight.setPower((0.5) * gamepad1.right_stick_y);
            robot.backLeft.setPower((0.5) * gamepad1.left_stick_y);
            robot.backRight.setPower((0.5) * gamepad1.right_stick_y);
        }
    }

    public void eating() {
        if (gamepad2.x) {
            robot.eatLeft.setPower(1.0);
            robot.eatRight.setPower(1.0);
        }
        else if (gamepad2.y) {
            robot.eatLeft.setPower(-1.0);
            robot.eatRight.setPower(-1.0);
        ...
        if (gamepad2.left_trigger >= 0.9) {
            robot.jongChulPark.setPosition(0.2);
        }
        else {
            robot.jongChulPark.setPosition(0.85);
        }
